...
[[ -z "$PROFILE_NAME" ]] && PROFILE_NAME="simone"
[[ -z "$PROFILE_SLUG" ]] && PROFILE_SLUG="simone"
[[ -z "$DCONF" ]] && DCONF=dconf
...
if which "$DCONF" > /dev/null 2>&1; then
    ...
    if [[ -n "`$DCONF list $BASE_KEY_NEW/`" ]]; then
        ...
        dset font "Anonymous Pro for Powerline 14"
        gconftool --set --type string /apps/gnome-terminal/global/default_profile "$PROFILE_NAME"
...
gset string font "Anonymous Pro for Powerline 14"
gconftool --set --type string /apps/gnome-terminal/global/default_profile "$PROFILE_NAME"
